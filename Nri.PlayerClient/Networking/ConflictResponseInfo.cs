...
public sealed class EntityRevisionStore
{
    ...
    public long GetRevision(string entityType, string entityId) => _items.TryGetValue($"{entityType}:{entityId}", out var state) ? state.Revision : 0;
    public void SetRevision(string entityType, string entityId, long revision, string source) => _items[$"{entityType}:{entityId}"] = new EntityRevisionState { EntityType = entityType, EntityId = entityId, Revision = revision, LastSeenUtc = DateTime.UtcNow, Source = source };
    public bool TryGetExpectedRevision(string entityType, string entityId, out long revision) { if (_items.TryGetValue($"{entityType}:{entityId}", out var state)) { revision = state.Revision; return true; } revision = 0; return false; }
    public void Clear() => _items.Clear();
    public void MarkStale(string entityType, string entityId) { if (_items.TryGetValue($"{entityType}:{entityId}", out var state)) { state.Source = "stale"; state.LastSeenUtc = DateTime.UtcNow; } }
}
